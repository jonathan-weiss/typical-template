...
object SummaryClassRenderer {
    ...
    fun renderTemplate(model: SummaryRenderModel): String {
        return """
          ...
          |${ model.fields.joinToString("") { field ->  """${ field.validationRules.joinToString("") { rule ->  """    // ${rule}
                  |""" } }${ if(field.isNullable) { """    val ${field.fieldName}: ${field.fieldType}?,
                  |""" } else if(field.isList) { """    val ${field.fieldName}: List<${field.fieldType}>,
                  |""" } else { """    val ${field.fieldName}: ${field.fieldType},
                  |""" } }""" } })
          |

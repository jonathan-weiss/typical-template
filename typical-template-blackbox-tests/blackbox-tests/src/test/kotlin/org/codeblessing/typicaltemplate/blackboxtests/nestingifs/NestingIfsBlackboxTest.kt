 ...
 * - `book + highlighted`   -> `BOOK` + `- highlighted` (outer `if`, nested `if`)
 * - `book + not highlighted` -> `BOOK` + `- plain` (outer `if`, nested `else`)
 * - `movie`                -> `MOVIE` (outer `else-if`)
 * - `neither`              -> `OTHER` (outer `else`)
 */
